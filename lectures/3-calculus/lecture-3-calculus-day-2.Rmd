...
- Derivatives can be calculated by taking limits of functions as they approach particular points. But usually we do this by applying a series of rules:
...
In this and many other cases, derivatives help us to do the maximization.
...
--
  ...
  - Zero $(f''(x^*) = 0)$: neither a minimum or a maximum (the function does not change concavity at this point); e.g., $f(x) = x^3$ when $x = 0$
...
This is the same antiderivative as in the definite integral, but we add an arbitrary constant $C$ (it disappears when taking the derivative).
